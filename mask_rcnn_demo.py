...
def instance_segmentation_demo():
    frame = cv.imread("D:/images/master.jpg")
    cv.imshow("frame", )
    blob = transform(frame)
    c, h, w = blob.shape
    input_x = blob.view(1, c, h, w)
    ...
    cv.imshow("intances segmentation demo", result)
    cv.imwrite("D:/master_test.png", result)

    # 简单背景替换
    back_ground = np.zeros_like(frame)
    back_ground[:,:,:] = (255, 0, 255)
    for row in range(h):
        for col in range(w):
            b, g, r = color_mask[row, col]
            if b > 0 or g > 0 or r > 0:
                back_ground[row, col] = (0, 0, 0)
    temp = cv.add(back_ground, frame, mask=mv[0])
    dst = cv.add(back_ground, temp)
    cv.imshow("background replacement", dst)
    cv.waitKey(0)
