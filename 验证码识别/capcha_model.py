...
from capcha_dataset import output_nums, CapchaDataset
...
if __name__ == "__main__":
    ...
    ds = CapchaDataset("D:\project\pytorch_stu\data\samples")
    ...
    # 损失函数
    mul_loss = torch.nn.MultiLabelSoftMarginLoss()  # 使用独热编码用这个损失函数
    index = 0
    for epoch in range(num_epochs):
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            #     acc += (predicted == test_labels.cuda()).sum()

            # predicted = torch.argmax(m_label_out_, 1)
            # print(m_label_out_, m_label_out_.shape)
            # print(oh_labels, oh_labels.shape)
            # acc += (m_label_out_ == oh_labels).sum()

            # update training loss
            train_loss += loss.item()
            if index % 100 == 0:
                print('step: {} \tTraining Loss: {:.6f} \tTraining Acc: {:.3f}'.format(index, loss.item(), acc))
            index += 1
        ...
        # 显示训练集与验证集的损失函数
        print(
            'Epoch: {} \tTraining Loss: {:.6f} Training Acc: {:.3f}'.format(epoch, train_loss, acc / num_train_samples))

    # save model
    model.eval()
    torch.save(model, 'capcha_recognize_model.pt')
